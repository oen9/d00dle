  ...
  def initBehavior(id: Int, lobbyManager: ActorRef[LobbyManager.LobbyManagerMsg]): Behavior[UserActorMsg] = Behaviors.receive { (ctx, msg) =>
    msg match {
      case RegisterOut(respondTo) =>
        ...
        behavior(id, respondTo, DEFAULT_USER_NAME, lobbyManager, findLobbyResponseMapper, None)
  ...
  def behavior(
    ...
    msg match {
      ...
      case InData(ChangeNickname(newNickname)) =>
        ...
        behavior(id, outRef, newNickname, lobbyManager, findLobbyResponseMapper, currentLobby)
      ...
      case InData(QuitLobby) =>
        ...
        behavior(id, outRef, nickname, lobbyManager, findLobbyResponseMapper, None)
      ...
      case FindLobbyResponseWrapper(LobbyManager.LobbyFound(lobbyRef, lobbyId)) =>
        ...
        behavior(id, outRef, nickname, lobbyManager, findLobbyResponseMapper, LobbyRef(lobbyRef, lobbyId).some)
